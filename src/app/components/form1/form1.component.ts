...
export class Form1Component implements OnInit {
  ...
  addGraph(nEvents, events) {
    ...
    if (nEvents.value == 2) { this.ncol = 6; }
    if (nEvents.value == 3) { this.ncol = 4; }
    if (nEvents.value >= 4) { this.ncol = 3; }
  ...
  setProbability(name, i, j, probability) {
    this.tProbability = { id: i + "" + j, name: name, positionN1: i, positionN2: j, probability: probability.value };
...
//Metodo para calcular las posibles opciones del calculo de bayes
  ...
  getOptions() {
        ...
        for (let j = 0; j < this.totalName.length; j++) {
          if (this.totalName[j] != null && this.totalName[j].positionN1 == 1) {
            this.options[this.nOption] = {
              id: "" + this.nOption + "",
              option: this.totalName[i].name + " / " + this.totalName[j].name + " (" + (i + 1) + ")",
              nProb1: this.totalName[i].name,
              prob1: 0,
              nProb2: this.totalName[j].name,
              prob2: 0,
              nprobParcial: this.totalName[i].name + " * " + this.totalName[j].name + " (" + (i + 1) + ")",
              probParcial: 0,
              nprobTotal: "",
              probTotal: 0,
              totalBayes: 0
    ...
    for (let i = 0; i < this.totalProbability.length; i++) {
      if (this.totalProbability[i] != null && this.totalProbability[i].positionN1 == 0) {
        for (let j = 0; j < this.totalProbability.length; j++) {
          if (this.totalProbability[j] != null && this.totalProbability[j].positionN1 == 1
            && this.totalProbability[j].positionN2 < (i * 2) + 2 && this.totalProbability[j].positionN2 > (i * 2) - 1) {
            ...
            this.options[this.nOption] = {
              ...
              probParcial: this.totalProbability[i].probability * this.totalProbability[j].probability,
    ...
    for (let i = 0; i < this.events.length; i++) {
      ...
      for (let j = 0; j < this.options.length; j++) {
        if (Number(this.options[j].id) % this.events[1] == i){
          ...
          if (Number(this.options[j].id)+2 < this.options.length) nTotal[t] += " + ";
        }
      }
      t++;
    }
    ...
    for (let i = 0; i < this.events.length; i++) {
      for (let j = 0; j < this.options.length; j++) {
        if (Number(this.options[j].id) < (i * this.events[1]) + (2) && Number(this.options[j].id) > (i * this.events[1]) - 1) {
          this.options[j] = {
    ...
    if (this.options[this.options.length-1].option != "Selecione una opcion") {
